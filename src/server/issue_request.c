	     ...
	     void (*func)(struct work_task *), struct work_task **ppwt)
{
	...
	conn = svr_connect(momaddr, momport, process_Dreply, ToServerDIS, PROT_TPP);
	if (conn < 0) {
	...
	pbs_errno = 0;
	rc = issue_Drequest(conn, request, func, &pwt, PROT_TPP);
	if ((rc == 0) && (func != release_req)) {
		/* work-task entry job related, link to the job's list */
		append_link(&pjob->ji_svrtask, &pwt->wt_linkobj, pwt);
		/* link to mom list as well */
...
issue_Drequest(int conn, struct batch_request *request, void (*func)(), struct work_task **ppwt, int prot)
{
	...
	switch (request->rq_type) {
		...
		case PBS_BATCH_FailOver:
			rc = put_failover(sock, request); /* we should never do tpp for this one */
			break;
		case PBS_BATCH_Cred:
			rc = PBSD_cred(conn,
	...
	if (rc) {
		...
		if (prot == PROT_TPP) {
			...
			ptask->wt_event2 = msgid;
			/*
			 * since its an tpp delayed task, remove it from the task_event list
			 * caller will add to moms deferred cmd list
			 */
			delete_link(&ptask->wt_linkall);
		}
		ptask->wt_aux2 = prot; /* 0 in case of non-TPP */
		*ppwt = ptask;
	}
 ...
 * 		with the TPP reply message. The TPP request for which this reply arrived
 ...
 *
 * @param[in] c - TPP stream on which reply/close arrived
 *
 * @return void
 */
void
process_DreplyTPP(int c)
{
	struct work_task *ptask;
	int rc;
	struct batch_request *request;
